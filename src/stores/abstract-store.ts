...
interface Store<T extends Record<PrimaryKeys | IndexKeys, string>, PrimaryKeys extends string = string, IndexKeys extends string = PrimaryKeys> {
  primaryKeys: Record<PrimaryKeys, Record<string, T>>;
  indexKeys: Record<IndexKeys, Record<string, T[]>>;
}

export abstract class AbstractStore<T extends Record<PrimaryKeys | IndexKeys, string>, PrimaryKeys extends string = string, IndexKeys extends string = PrimaryKeys> {
  ...
  constructor(primaryKeys: PrimaryKeys[], indexKeys: IndexKeys[]) {
    ...
    this.store = map<Store<T, PrimaryKeys, IndexKeys>>({
      primaryKeys: primaryKeys.reduce((acc: Record<PrimaryKeys, Record<string, T>>, key: PrimaryKeys) => {
        acc[key] = {} as Record<string, T>;
        return acc;
      }, {} as Record<PrimaryKeys, Record<string, T>>),
      indexKeys: indexKeys.reduce((acc: Record<IndexKeys, Record<string, T[]>>, key: IndexKeys) => {
        acc[key] = {} as Record<string, T[]>;
        return acc;
